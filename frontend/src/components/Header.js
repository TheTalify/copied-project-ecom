...
const Header = () => {
  ...
  return (
    <header className='h-16 shadow-md bg-gradient-to-r from-green-200 to-blue-200 fixed w-full z-40'>
              ...
              <div className='absolute bg-white bottom-0 top-11 h-fit p-2 shadow-lg rounded'>
                <nav>
                  {user?.role === ROLE.ADMIN && (
                    <Link to={"/admin-panel/all-products"} className='whitespace-nowrap hidden md:block hover:bg-gray-100 p-2' onClick={() => setMenuDisplay(prev => !prev)}>Admin Panel</Link>
                  )}
                </nav>
              </div>
            )}
